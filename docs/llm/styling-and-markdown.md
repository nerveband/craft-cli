...
### Important: markdown replacement does not preserve block-only styling

Whole-doc `craft update --mode replace` and markdown insertion paths are markdown-based. They are good for structural rewrites, headings, lists, callouts, captions, and inline markdown features that can be represented in markdown.

Block-level JSON fields such as `color`, `font`, `textAlignment`, card layout, task state, media/embed fields, comments, revert anchors, and richer MCP style fields are not markdown syntax. If you replace the whole document as markdown, Craft deletes and recreates content blocks, so those block-only styles/state can be lost and block IDs will change.
...
Always send the original source/brand hex values in styling passes. Do not read Craft's stored adjusted color and feed that adjusted value back as the new source of truth.
...
Both quotes and callouts can have a `color` field (hex `#RRGGBB`) to tint the decoration. Craft may store a nearby adjusted color rather than the exact input hex.
...
Text blocks can accept a `color` field with a `#RRGGBB` hex value. Craft may store an adjusted value for readability and palette compatibility, with a dark-mode variant generated by Craft. Treat this as Craft server behavior: the CLI sends your hex, but `GET /blocks` may return a nearby Craft-adjusted color.
...
For brand-sensitive styling, keep the brand hexes in your source data and resend those original values when reapplying styles. Do not repeatedly reuse the adjusted value returned by Craft, because repeated writes can drift through Craft's color normalization.
